...
let initCrossStorage = false;
const performInitCrossStorage = async () => {
  if (isLocal()) {
    const clientOnly = await import('../components/clientOnly');
    clientOnly.handleCrossOrigin().then(() => {
      setTimeout(() => {
        initCrossStorage = true;
        // performRedirectUser();
      }, MINIMUM_WAIT_TIME);
    });
  }
};
performInitCrossStorage();

const performRedirectUser = async () => {};
...
const ActionPerformer = React.memo(() => {
  ...
  return (
    <Typography style={{ textAlign: 'center' }}>
      {initCrossStorage ? 'Redirecting to Teyvat.moe...' : 'Migrating local storage migration...'}
    </Typography>
  );
});
